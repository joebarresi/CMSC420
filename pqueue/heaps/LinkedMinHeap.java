...
public class LinkedMinHeap<T extends Comparable<T>> implements MinHeap<T> {
	...
	private class MinHeapNode {
		...
		MinHeapNode(T data, MinHeapNode lChild, MinHeapNode rChild, MinHeapNode parent) {
			this.data = data;
			this.lChild = lChild;
			this.rChild = rChild;
			this.parent = parent;
	...
	public LinkedMinHeap(T rootElement) {
		size = 1;
		root = new MinHeapNode(rootElement, null, null, null);
		tail = root;
	}
	...
	public void insert(T element) {
		if (size == 0) {
			root = new MinHeapNode(element, null, null, null);
			tail = root;
		} else {
			if (tail.lChild == null) {
				tail.lChild = new MinHeapNode(element, null, null, tail);
			} else if (tail.rChild == null) {
				tail.rChild = new MinHeapNode(element, null, null, tail);
				tail = tail.rChild;
			} 
		}
		size += 1;
