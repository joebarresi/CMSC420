...
public class SeparateChainingHashTable implements HashTable{
    ...
    public Probes put(String key, String value) {
    	// May need to deal with enlarging if over capacity
    	
        int spot = hash(key);
        count += 1;
        
        if (table[spot] == null) {
        	table[spot] = new KVPairList(key, value);
        } else {
        	table[spot].addBack(key, value);
        }
    ...
    public Probes remove(String key) {
        ...
        KVPairList spot = table[hash(key)];
        
        Probes cur = spot.removeByKey(key);
    ...
    public boolean containsKey(String key) {
         return table[hash(key)].getValue(key) != null;
    }

    @Override
    public boolean containsValue(String value) {
    	for (KVPairList cur: table) {
    		if (cur.containsValue(value) == true) {
    			return true;
    		}
    	}
    ...
    public void enlarge() {
    	...
    	for (KVPairList cur: temp) {
    		if (cur != null)
	    		for (KVPair cur2 : cur) {
	    			put(cur2.getKey(), cur2.getValue());
	    		}
    	}
    }
